...
const Setting = () => {
  ...
  const AddAssignProduct = async (e) => {
    ...
    try {
      if (editAssignproductIndex) {
        ...
        const result = await axios.post(
          `${base_url}/setting/assignproduct`,
          { divisionId: divisionIndex, assignProduct: assignProduct }
        );
  ...
  const SavePincodeData = async () => {
    ...
    try {
      const result = await axios.post(
        `${base_url}/pincode/create-pincode`,
        {
  ...
  async function UpdatePincode() {
    try {
      const result = await axios.put(
        `${base_url}/pincode/update-pincode`,
        {
  ...
  const handleAutoEmailDelete = async (id) => {
    try {
      await axios.delete(
        `${base_url}/backup/delete-backup-emails/${id}`
      );
      fetchBackup();
    } catch (err) {
      console.log("internal error", err);
  ...
  const fetchBackup = async () => {
    try {
      const result = await axios.get(
        `${base_url}/backup/get-backup-emails`
      );
      console.log("autobakup", result.data.result);
      setFetchBackupEmail(result.data.result);
    } catch (err) {
      console.log("internal error", err);
    }
  };
            ...
            >
              <div className="general-setting-div">
                <div className="role-name-div">
                  ...
                  {/* <button onClick={AddRoleName}>Add</button> */}
