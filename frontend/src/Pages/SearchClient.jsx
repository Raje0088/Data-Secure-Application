...
const SearchClient = () => {
  ...
  useEffect(() => {
    const showExcelData = async () => {
      ...
      try {
        ...
        const totalCount = clientChecker.filter((item) => item.setter === true);
        setTotalDone(totalCount.length);
        const userChecker = await Promise.all(
          obj.map(async (doc) => {
            ...
            return {
              id: doc.client_id,
              setter: !!isPresent.data.result,
            };
          })
        );
        // console.log("checkuser", userChecker);

        setTrueUserSetter(userChecker);
        setTrueClientSetter(clientChecker);
  ...
  // const handleFieldsCheckboxChange = (e,opticalName,key)=>{
  //   const isChecked = e.target.checked
  //   if(isChecked){
  //     if(selectedOpticalName.length >= 3){
  //       alert("You can only select upto to 3 opticalName")
  //     }
  //           const updatedOpticalName = [...selectedOpticalName, { key, opticalName }];
  //     setSelectedOpticalName(updatedOpticalName);
  //     setFields((prev) => ({
  ...
  return (
    <div className={styles.main}>
      ...
      {/* //BACKUP DUPLICATRE RECORD */}
      {/* {!isLoading && isDuplicate && duplicateRecord.result?.length > 0 && (
        <div className={styles.tablediv}>
          <div className={styles.duplicaterecorddiv}>
            <h2>
              Total Duplicate Groups :{" "}
              {duplicateRecord.limit < totalDuplicateRecordCount
                ? duplicateRecord.limit
          ...
          {duplicateRecord.result.map((group, idx) => (
            <div className={styles.tables} key={idx}>
              <h2>Total Match Duplicated : {group.count}</h2>
              <table>
                <thead>
                  <tr>
                    <th>
                      <span id={styles.selectAll}>
                        <input
                          ...
                          className={styles["checkbox-input-table"]}
                          checked={
                            duplicateRecord?.result[idx]?.doc?.every((doc) =>
                              selectedClients.includes(doc.client_id)
                            ) && group.doc.length > 0
                          }
                          onChange={() => {
