...
const rawDataDump = async (req, res) => {
    try {
        ...
        const filePath = path.join(__dirname, "../uploadExcel", fileName);
...
const filterRawData = async (req, res) => {
    try {
        ...
        const result = await rawDataModel.find(filters).sort({ client_id: 1 }).skip(skip).limit(limit)
        const totalCount = await rawDataModel.countDocuments(filters)
        res.status(200).json({ message: "Client details filter result", page: Number(page), limit: limit, totalCount: totalCount, resultCount: result.length, result })
    } catch (err) {
        console.log("internal error", err)
        res.status(500).json({ message: "internal error", err: err.message })
    }
...
const rawSampleFile = async (req, res) => {
    try {
        ...
        const filePath = path.join(__dirname, `../sampleFile`, file)
