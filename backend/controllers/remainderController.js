...
const startRemainder = () => {
    cron.schedule("* * * * *", async () => {
        try {
            ...
            const results = await clientHistoryModel.find({ "completion_db.status": "Done", "completion_db.newExpectedDate": date })
            if (!results) {
                console.log("No matching client history found");
                return;
            }
            for (const result of results) {
                const doe = await remainderModel.findOneAndUpdate(
                    {
                        client_id: result.client_id, date_db: date, stage_db: result.completion_db.newStage,
                    },
                    {
                        $set: {
                            status_db: true,
